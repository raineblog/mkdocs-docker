...
class CachedURLFetcher(URLFetcher):
    ...
    def get_total_size(self) -> int:
        ...
        return sum(
            len(item["content"])
            for item in self.cache_pool.values()
            if "content" in item
        )
        ...
        if response is not None:
            try:
                ...
                raw_body = (
                    ...
                    or getattr(response, "string", None)
                    or getattr(response, "file_obj", None)
                )
...
class MlibDownloader:
    ...
    def __init__(self, default_base_url: Optional[str] = None):
        ...
        print(
            f"🚀 MlibDownloader V7 初始化 | 根目录: {default_base_url or 'auto'}",
            flush=True,
        )
        ...
        try:
            ...
            if resp and "font-face" in url.lower() or ".css" in url.lower():
                try:
                    ...
                    if font_urls:
                        print(
                            f"   ⚓ 探测到 {len(font_urls)} 个关联字体资源，正在并行预载...",
                            flush=True,
                        )
            ...
            print(
                f"   ✅ 资产预热完成 (当前缓存: {format_size(self._fetcher.get_total_size())})",
                flush=True,
            )
        except Exception as e:
    ...
    def add_task(
        self, html_source: str, pdf_path: str, base_url: Optional[str] = None
    ) -> None:
        """添加待处理任务"""
        print(f"➕ 添加任务: {pathlib.Path(pdf_path).name}", flush=True)
        self._task_queue.append(
            (html_source, pdf_path, base_url or self.default_base_url)
        )
    ...
    def start_tasks(self) -> None:
        ...
        print(
            f"\n▶️ 开始生产 {total} 份 PDF | 初始缓存: {format_size(self._fetcher.get_total_size())}",
            flush=True,
        )
        ...
        for i, (src, dst, base) in enumerate(self._task_queue, 1):
            ...
            try:
                ...
                html = (
                    HTML(filename=str(src_p), **html_kwargs)
                    if src_p.is_file()
                    else HTML(url=src, **html_kwargs)
                )
    ...
    def _report_stats(self) -> None:
        ...
        print(
            f"   • 全局 fetcher 缓存: {len(self._url_cache_pool)} 项 | 大小: {format_size(size)}",
            flush=True,
        )
